...
function playSound(soundFile, volume = 1) {
  ...
  if (isWindows) {
    ...
    try {
      if (ext === '.wav') {
        const p = spawn('powershell.exe', ['-c', `(New-Object System.Media.SoundPlayer '${soundFile.replace(/'/g, "''")}').PlaySync()`], {
          detached: true,
          stdio: 'ignore'
        });
        p.unref();
        ...
        const p = spawn('cmd.exe', ['/c', 'start', '/b', 'mshta', `vbscript:CreateObject("WMPlayer.OCX.7").OpenPlayer("${soundFile.replace(/\\/g, '\\\\').replace(/"/g, '\\\\"')}").Play:close`], {
          detached: true,
          stdio: 'ignore'
        });
        p.unref();
